"""
fast/llm.py — 调 Kimi (Moonshot) 生成故事 JSON。
...
# 精简音色提示：只给分类和代表性示例，让 Kimi 按特征选择
VOICE_CATALOG = """
...
def generate_story(theme: str, tone: str = "留白向", ai_style: str = "陪伴感",
                   characters: str = "一个在外地工作的中年人") -> dict:
    prompt = f"""请创作一个对话故事：
主题：{theme}
基调：{tone}
AI形象：{ai_style}
...
直接输出 JSON。"""
    ...
    try:
        ...
        dump = Path("/tmp/kimi_raw.json")
        dump.write_text(raw, encoding="utf-8")
        raise RuntimeError(
            f"Kimi 返回非合法 JSON ({e})。原始响应已存到 {dump}，"
            f"finish_reason={resp.choices[0].finish_reason}"
        ) from e

    if not story.get("slug"):
        story["slug"] = re.sub(r"[^a-z0-9-]", "", story.get("title", "story").lower()) or "story"
    return story
